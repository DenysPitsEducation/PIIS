...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax(self, game_state: GameState, agent, depth):
        ...
        if agent == 0:
            ...
            minValue = float("inf")
            ghostsNum = game_state.getNumAgents() - 1
            ...
            if agent == ghostsNum:
                ...
                next = agent + 1
            ghostActions = game_state.getLegalActions(agent)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def alphabeta(self, game_state: GameState, agent, depth, alpha, beta):
        if game_state.isLose() or game_state.isWin() or depth == 0:
            return [self.evaluationFunction(game_state)]
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def expectimax(self, game_state, agent, depth):
        ...
        if agent == 0:
            ...
            min_value = 0
            ghosts_count = game_state.getNumAgents() - 1

            if agent == ghosts_count:
                depth -= 1
                next = 0
            else:
                next = agent + 1
